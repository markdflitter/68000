...
public:
	...
	static unsigned char* serialise (BootTableEntry::Ptr bte, unsigned char* p);
	...
	template <class T> static unsigned char* serialise (const std::vector<T>& v, unsigned char* p)
	{
		size_t num = v.size ();
		p = (unsigned char*) serialise (num, p);
	...
	static unsigned char* deserialise (BootTableEntry::Ptr bte, unsigned char* p);
	...
	template <class T> static unsigned char* deserialise (std::vector <T>& v, unsigned char* p)
	{
		size_t num = 0;
		p = deserialise (num, p);
	...
	template <class T> static unsigned char* deserialise (std::vector <mdf::shared_ptr <T> >& v, unsigned char* p)
	{
		size_t num = 0;
		p = deserialise (num, p);
